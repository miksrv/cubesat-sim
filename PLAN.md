...
(I2C-мост SC16IS752) на Heltec WiFi LoRa 32 V4 со стоковой прошивкой Meshtastic,
подключённый к Raspberry Pi по UART.

**Дата составления:** 23 августа 2026
**Статус:** закрыт. Этапы 1–5 выполнены 23 августа 2026 — канал Pi ↔ Heltec ↔
эфир поднят и проверен в обе стороны. Этап 6 выполнен 24 августа 2026: драйвер
живёт в `src/cubesat/hal/rpi/meshtastic_radio.py`, сервис — в
`src/cubesat/comms/`, размер пакета решён компактным маяком. **И драйвер, и
сервис отработали на Raspberry Pi 31 августа 2026** в профиле `HOSTED`: нода
отвечала, маяк ушёл в эфир, команда с телефона доехала до `cubesat/command` и
получила ответ. Последнее незакрытое — пакет через третью ноду — измерено
2 сентября 2026 (арифметика хопов) и 5 сентября 2026 (ретрансляция закрытого
канала чужим узлом); см. [`docs/hardware-heltec-lora32-v4.md`](docs/hardware-heltec-lora32-v4.md).

> **Этап 6 — это фаза P0 переписывания.** См. [`ROADMAP.md`](ROADMAP.md) и
> [`docs/concept.md`](docs/concept.md): радиоканал блокирует профили `EXPO` и
> `FLIGHT`, где LoRa — единственный путь наружу и единственный путь назад.
> Пункт G10 старого роадмапа (проверка регистров SC16IS752 через SDR) отменён:
> платы больше нет в схеме, код не проверяется, а удаляется. В новой раскладке
> файл называется `src/comms/mesh.py`, а не `lora.py`, и `COMMS` после разреза
> не владеет базой — запись ушла в отдельный сервис `DHS`.
...
Это фаза **P0** переписывания (см. [`ROADMAP.md`](ROADMAP.md)). Названия файлов —
из целевой раскладки в [`README.md`](README.md#directory-structure), а не из
текущего кода.

- [ ] `src/comms/lora.py` → **`src/comms/mesh.py`**: драйвер поверх
      `meshtastic.serial_interface.SerialInterface`, приём через колбэк
      `pub.subscribe(on_receive, "meshtastic.receive")`. Ни `smbus2`, ни сырого
      `pyserial`. Своё кадрирование `length + payload + CRC-16-CCITT` не нужно:
      Meshtastic сам делает кадрирование, CRC, ретраи и шифрование
- [x] **Размер пакета — решено: компактный маяк, а не чанкование.** Одно сообщение —
      одно полное наблюдение; потерянный чанк обнулял бы пакет целиком. Эфирное
      время на LONG_FAST дорого и ограничено по duty cycle. А радио обязано
      ответить на вопрос «жив и где», а не передать всю запись — она у DHS и
      забирается по возвращении в сеть. Формат читаем в телефонном приложении
      Meshtastic, поля при нехватке места отбрасываются целиком по приоритету,
      усечения нет никогда. Подробности: Сообщение Meshtastic — до 240 байт, а полный
      пакет телеметрии — сотни. Сейчас `lora.py:29-31` молча режет payload до
      28 байт, то есть `service.py:293` отправляет обрезанный мусор. Решать до
      первого включения радио: компактный beacon-набор полей **или** чанкование.
      Профили `EXPO` и `FLIGHT` без этого не имеют смысла — там LoRa
      единственный канал наружу
- [ ] Первый `--info` через UART может вернуть пусто (см. «Известные грабли») —
      предусмотреть повтор при инициализации, а не падать
- [ ] `src/common/config.py` + `config/config.yaml` — убрать `LORA_I2C_ADDRESS`
      и `GPS_PORT`/`GPS_BAUDRATE` (A9G тоже выведен), добавить `LORA_PORT`
      (`/dev/serial0`), `LORA_BAUDRATE` (115200, не опционально), таймаут
- [ ] `requirements.txt` — добавить `meshtastic`; мок в `tests/conftest.py`
      рядом с остальными подменами `sys.modules`
- [ ] `tests/test_comms_lora.py` — сейчас мокает `smbus`, переделать под
      `meshtastic`
- [x] `crc16_ccitt()` в `src/common/utils.py` теряет последнего потребителя —
      удалить вместе с его тестом. Сделано вместе с удалением старой кодовой базы:
      ни функции, ни модуля в новом дереве нет, кадрирование целиком за Meshtastic
- [x] `docs/hardware-heltec-lora32-v4.md` — новый файл; строка в таблице
      Hardware в `README.md`
- [x] `ROADMAP.md` — G10 отменён, а не переформулирован: платы нет в схеме
- [ ] Для двустороннего канала на земле нужен SX1262-приёмник у ground station.
      Отдельная задача, вне этого репозитория

Чего в этом этапе **нет**, хотя оно рядом: `COMMS` после разреза не владеет
базой — запись телеметрии уезжает в сервис `DHS` (фаза P3). Не тащить сюда.
